...
public final class ValueMaskers {
    ...
    /**
     * Provides information about the {@link ValueMasker} implementation. Which is useful for debugging and testing
     * purposes.
     *
    ...
    public static ValueMasker.AnyValueMasker with(String value) {
        ...
        return describe(replacement, context -> context.replaceBytes(0, context.byteLength(), replacementBytes, 1));
    }
    ...
    public static ValueMasker.AnyValueMasker withNull() {
        ...
        return describe(
                "null (literal)", context -> context.replaceBytes(0, context.byteLength(), replacementBytes, 1));
    }
     ...
     * <p>For example, {@literal "maskMe": "secret" -> "maskMe": "******"}.
     *
     * <p>Note: this implementation only replaces visible characters with a mask, meaning that JSON escape character
     * ('\') will not count towards the length of the masked value and the unicode characters ({@code \\u1000}),
     * including 4-byte UTF-8 characters ({@code \\uD83D\\uDCA9}), will only count as a single character in the masked
     * value.
     */
    public static ValueMasker.StringMasker eachCharacterWith(String value) {
        String replacement = Utf8Util.jsonEncode(value, false);
        byte[] replacementBytes = replacement.getBytes(StandardCharsets.UTF_8);
        return describe(String.format("every character as %s", replacement), context -> {
            /*
    ...
    public static ValueMasker.NumberMasker eachDigitWith(String value) {
        ...
        return describe(String.format("every digit as string: %s", replacement), context -> {
    ...
    /**
     * Does not mask a target value (no-operation). Can be used if certain JSON value types do not need to be masked,
     * for example, not masking booleans or numbers.
     *
    ...
    /**
     * Masks a target string value (containing an email) while keeping some number of the prefix or suffix characters
     * and the ability to keep the domain unmasked.
     *
     * <p>For example:
     *
     * <ul>
     *   <li>{@literal "maskMe": "agavlyukovskiy@gmail.com" -> "maskMe": "***@gmail.com"}
     *   <li>{@literal "maskMe": "agavlyukovskiy@gmail.com" -> "maskMe": "ag***"}
     *   <li>{@literal "maskMe": "agavlyukovskiy@gmail.com" -> "maskMe": "ag***@gmail.com"}
     *   <li>{@literal "maskMe": "agavlyukovskiy@gmail.com" -> "maskMe": "ag***iy@gmail.com"}
     * </ul>
     *
     ...
     */
    public static ValueMasker.StringMasker email(
            int keepPrefixLength, int keepSuffixLength, boolean keepDomain, String mask) {
        byte[] replacementBytes = mask.getBytes(StandardCharsets.UTF_8);
        return describe(
                String.format(
                        "email, keep prefix: %s, keep suffix: %s, keep domain: %s",
                        keepPrefixLength, keepSuffixLength, keepDomain),
                context -> {
     ...
     * a JSON string the value the function will receive a JSON encoded value as it appears in the JSON, including the
     * opening and closing quotes, and the value containing escaped the control characters (e.g. {@code \n}, {@code \t},
     * etc.), quotation marks ({@code "}), escape character itself ({@code \}), and unicode-encoded characters
     * ({@code \\u0000}).
     *
     * <p>Consequently, the return value of the provided function must be a valid JSON encoded literal (of any JSON
     * type), otherwise the masking will result in an invalid JSON. If the return value is {@code null}, the target
     * value will be replaced with {@code null} JSON literal.
     *
     * <p>It is strongly advised to use an equivalent function {@link ValueMaskers#withTextFunction(Function)} which
     * operates on a decoded string values and can never produce an invalid JSON.
     *
     ...
     * </table>
     *
     * <p>Note: usually the {@link ValueMasker} operates on a byte level without parsing JSON values into intermediate
     * objects. This implementation, however, needs to allocate a {@link String} before passing it into the function and
     * then turn it back into a byte array for the replacement, which introduces some performance overhead.
     *
    ...
    public static ValueMasker.AnyValueMasker withRawValueFunction(Function<String, @Nullable String> masker) {
        return describe(String.format("withRawValueFunction (%s)", masker), context -> {
     ...
     * without the quotes.
     *
     * <p>A non-null return value of the provided function will be encoded into a JSON string regardless of the JSON
     * type of the original value. Any character that MUST be escaped (as per <a
     * href="https://datatracker.ietf.org/doc/html/rfc8259#section-7">RFC 8259, section 7</a>) will be escaped.
     ...
     * intermediate objects. This implementation, however, needs to allocate a {@link String} before passing it to the
     * {@link Function} and then turn it back into a byte array for the replacement, which introduces some performance
     * overhead.
     */
    public static ValueMasker.AnyValueMasker withTextFunction(Function<String, @Nullable String> masker) {
        return describe(String.format("withTextFunction (%s)", masker), context -> {
                ...
                while (encodedIndex < valueEndIndex) {
                    ...
                    if (originalByte != '\\') {
                        ...
                        switch (originalByte) {
                            ...
                            case 'u': // Decode hexadecimal encoded unicode character into
                                ...
                                try {
                                    ...
                                    if (unicodeHexBytesAsChar < 0x80) {
                                        ...
                                        decodedBytes[decodedIndex++] =
                                                (byte) (0x80 | ((unicodeHexBytesAsChar >> 6) & 0x3f));
                                        decodedBytes[decodedIndex++] = (byte) (0x80 | (unicodeHexBytesAsChar & 0x3f));
                                    } else {
                                        ...
                                        if (Character.isHighSurrogate(
                                                        unicodeHexBytesAsChar) // first surrogate must be the high
                                                // surrogate
                                                ...
                                                && context.getByte(encodedIndex)
                                                        == '\\' // the high surrogate must be followed by a low
                                                // surrogate (starting with \\u)
                                        ...
                                        if (codePoint < 0) {
                                            // default String behaviour is to replace invalid surrogate pairs with the
                                            // "replacement character"
                                            // (https://en.wikipedia.org/wiki/Specials_(Unicode_block)#Replacement_character)
                                            // but from the JSON perspective, it's better to throw an
                                            // InvalidJsonException
                                            throw context.invalidJson(
                                                    String.format(
                                                            "Invalid surrogate pair '%s'",
                                                            context.asString(
                            ...
                            default:
                                throw context.invalidJson(
                                        String.format("Unexpected character after '\\': '%s'", (char) originalByte),
                                        encodedIndex);
                        }
                    }
                }
