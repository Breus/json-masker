...
class FuzzingTest {
    ...
    @ParameterizedTest
    @ValueSource(ints = {100000}) // number of tests
    void fuzzTestNoFailuresSingleTargetLoopAlgorithm(int amountOfTests) {
        for (int i = 0; i < amountOfTests; i++) {
            ...
            RandomJsonGenerator randomJsonGenerator = new RandomJsonGenerator(RandomJsonGeneratorConfig.builder().createConfig());
            JsonNode randomJsonNode = randomJsonGenerator.createRandomJsonNode();
            Assertions.assertDoesNotThrow(() -> singleTargetMasker.mask(randomJsonNode.toPrettyString()), randomJsonNode.toPrettyString());
        }
    }
    ...
    void fuzzTestTwoTargets(int amountOfTests) {
        for (int i = 0; i < amountOfTests; i++) {
            ...
            Assertions.assertEquals(jacksonMaskingOutput, keyContainsOutput, "Failed for input: " + randomJsonNodeString);
        }
    }
}
