...
class StreamingModeTest {
    ...
    void shouldHandleBufferingSuccessfully(
            Integer initialBufferSize, Set<String> maskKeys, String json, String expectedResult) {
        JsonMaskingConfig config =
                JsonMaskingConfig.builder().maskKeys(maskKeys).build();
        JsonMaskingConfigTestUtil.setBufferSize(
                JsonMaskingConfig.builder().maskKeys(maskKeys).build(), initialBufferSize);
    ...
    void shouldThrowWriteErrorOnOutputStreamFailureDuringFlush() {
        ...
        InputStream inputStream =
                new ByteArrayInputStream("{\"doNotMask\": [0], \"mask\": \"a\"}".getBytes(StandardCharsets.UTF_8));
    ...
    private static Stream<Arguments> getBufferingSituations() {
        return Stream.of(
                ...
                Arguments.of(1024, Set.of("mask"), "{\"mask\":\"value\"}", "{\"mask\":\"***\"}"),
                ...
                Arguments.of(
                        ...
                        "{\"doNotMask\": \"value\", \"mask\": \"***\"}"),
                // process json with re-reading the buffer, token start index is registered for a value.
                // the buffer ends in registered "veryLongValue" value. The value length is equal to a threshold
                // (quarter size of the buffer)
                Arguments.of(
                        12,
                        Set.of("mask"),
                ...
                // process json with re-reading the buffer, token start index is registered for a value.
                // the buffer ends in registered "veryLongValue" value. The registered value length is larger than a
                // threshold (quarter size of the buffer)
                Arguments.of(
                        12,
                        Set.of("mask"),
                        "{\"mask\":\"veryLongValue\", \"doNotMask\": \"value\"}",
                ...
                Arguments.of(
                        12,
                        Set.of(
                                "maskThisVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryVeryLongKey"),
