...
public final class ParseAndMaskUtil {
    ...
    static JsonNode mask(JsonNode jsonNode, JsonMaskingConfig jsonMaskingConfig) {
        if (jsonMaskingConfig.isInAllowMode() && !jsonNode.isArray() && !jsonNode.isObject()) {
            return maskJsonValue(
                    jsonNode,
                    jsonMaskingConfig.getDefaultConfig(),
                    jsonMaskingConfig,
                    jsonMaskingConfig.getTargetKeys());
        }
        ...
        if (jsonMaskingConfig.caseSensitiveTargetKeys()) {
            ...
            casingAppliedTargetJsonPathKeys = jsonMaskingConfig.getTargetJsonPaths().stream()
                    .map(JsonPath::toString)
                    .collect(Collectors.toSet());
        } else {
            casingAppliedTargetKeys = jsonMaskingConfig.getTargetKeys().stream()
    ...
    static JsonNode mask(
        ...
        if (jsonNode instanceof ObjectNode objectNode) {
            ...
            for (String propertyName : propertyNames) {
                ...
                String casingAppliedJsonPathKey =
                        jsonMaskingConfig.caseSensitiveTargetKeys() ? jsonPathKey : jsonPathKey.toLowerCase();
                if ((jsonMaskingConfig.isInMaskMode()
                                && isTargetKey(
                                        casingAppliedJsonPathKey,
                                        casingAppliedTargetKeys,
                                        casingAppliedTargetJsonPathKeys))
                    ...
                    objectNode.replace(
                        ...
                        || !isTargetKey(
                                casingAppliedJsonPathKey, casingAppliedTargetKeys, casingAppliedTargetJsonPathKeys)) {
                    mask(
                            jsonNode.get(propertyName),
                            jsonMaskingConfig,
                            jsonPathKey,
                            casingAppliedTargetKeys,
                            casingAppliedTargetJsonPathKeys);
    ...
    private static JsonNode maskJsonValue(
        ...
        return switch (jsonNode.getNodeType()) {
            ...
            case OBJECT ->
                maskObjectNodeValue((ObjectNode) jsonNode, config, jsonMaskingConfig, casingAppliedTargetKeys);
            default -> jsonNode;
        };
    }
    ...
    private static JsonNode maskBooleanNode(BooleanNode booleanNode, KeyMaskingConfig config) {
        String maskedValue =
                ByteValueMaskerContext.maskBooleanWith(booleanNode.booleanValue(), config.getBooleanValueMasker());
    ...
    private static JsonNode maskNumericNode(NumericNode numericNode, KeyMaskingConfig config) {
        String maskedValue =
                ByteValueMaskerContext.maskNumberWith(numericNode.numberValue(), config.getNumberValueMasker());
        return DEFAULT_JSON_MAPPER.readTree(maskedValue);
    }
    ...
    private static ObjectNode maskObjectNodeValue(
        ...
        objectNode.forEachEntry((key, value) -> {
            if (jsonMaskingConfig.isInAllowMode()
                    && casingAppliedTargetKeys.contains(
                            jsonMaskingConfig.caseSensitiveTargetKeys() ? key : key.toLowerCase())) {
