...
final class KeyMatcherTest {
    ...
    void shouldMatchKeysCaseInsensitiveByDefault() {
        KeyMatcher keyMatcher =
                new KeyMatcher(JsonMaskingConfig.builder().maskKeys(keys).build());
        for (String key : keys) {
    ...
    void shouldBeAbleToSearchByOffset() {
        KeyMatcher keyMatcher =
                new KeyMatcher(JsonMaskingConfig.builder().maskKeys("maskMe").build());
        ...
        assertThat(keyMatcher.getMaskConfigIfMatched(bytes, 0, bytes.length, pointer, null))
                .isNotNull();
        assertThat(keyMatcher.getMaskConfigIfMatched(bytesWithPadding, 2, bytes.length, pointer, null))
                .isNotNull();
    }
    ...
    void shouldNotMatchPrefix() {
        KeyMatcher keyMatcher = new KeyMatcher(
                JsonMaskingConfig.builder().maskKeys(Set.of("maskMe", "test")).build());
    ...
    void shouldAllowVeryLargeKeys() {
        ...
        KeyMatcher keyMatcher =
                new KeyMatcher(JsonMaskingConfig.builder().maskKeys(Set.of(key)).build());
        assertThatConfig(keyMatcher, key).isNotNull();
    }
    ...
    @Test
    void printsEmpty() {
        JsonMaskingConfig config = JsonMaskingConfig.builder().allowKeys().build();
        KeyMatcher keyMatcher = new KeyMatcher(config);
        assertThat(keyMatcher.printTree()).isEqualTo("\n");
    }
    ...
    void compressPreInitTrie() {
        ...
        KeyMatcher keyMatcher = new KeyMatcher(
                JsonMaskingConfig.builder().maskKeys("breus", "bruce").build());
        ...
        // Prefix index = 2, because b = 0, r = 1, and 2 would be 'e', but it will be looked-up in the next node because
        // prefixIndex == currentRadixNode.length (2)
        assertThat(trieNode.child((byte) 'e', 2))
                .isNotNull()
                .extracting("prefixLowercase")
                .isEqualTo("us".getBytes(StandardCharsets.UTF_8));
        assertThat(trieNode.child((byte) 'u', 2))
                .isNotNull()
                .extracting("prefixLowercase")
                .isEqualTo("ce".getBytes(StandardCharsets.UTF_8));
    }
    ...
    void isUnicodeEncodedCharacter(byte[] bytes, int fromIndex, int toIndex, boolean isEncodedCharacter) {
        assertThat(KeyMatcher.isUnicodeEncodedCharacter(bytes, fromIndex, toIndex))
                .isEqualTo(isEncodedCharacter);
    }
