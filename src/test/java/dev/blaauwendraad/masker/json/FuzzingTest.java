...
final class FuzzingTest {
    ...
    void fuzzingAgainstParseAndMaskUsingJackson(JsonMaskingConfig jsonMaskingConfig) {
        long timeLimit =
                FuzzingDurationUtil.determineTestTimeLimit(jsonMaskingConfigs().count());
        Instant startTime = Instant.now();
        int randomTestExecuted = 0;
        Set<String> allKeys = new HashSet<>(jsonMaskingConfig.getTargetKeys());
        allKeys.addAll(jsonMaskingConfig.getTargetJsonPaths().stream()
                .map(JsonPath::getQueryArgument)
                .collect(Collectors.toSet()));
        RandomJsonGenerator randomJsonGenerator = new RandomJsonGenerator(
                RandomJsonGeneratorConfig.builder().setTargetKeys(allKeys).createConfig());
        ...
        while (Duration.between(startTime, Instant.now()).toMillis() < timeLimit) {
            ...
            for (JsonFormatter formatter : JsonFormatter.values()) {
                ...
                String jacksonMaskingOutput = ParseAndMaskUtil.mask(randomJsonString, jsonMaskingConfig)
                        .toString();
                try {
                    JsonMaskerTestUtil.assertJsonMaskerApiEquivalence(
                            masker, randomJsonString, jacksonMaskingOutput, true);
                } catch (Exception e) {
        ...
        System.out.printf(
                "Executed %d randomly generated test scenarios in %d milliseconds%n", randomTestExecuted, timeLimit);
    }
