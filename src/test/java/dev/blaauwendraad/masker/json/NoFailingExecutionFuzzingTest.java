 ...
 *
 * <p>For each {@link JsonMaskingConfig}, random JSON inputs are generated against which the masker runs and the only
 * thing that is tested it doesn't cause an exception or gets stuck in a loop.
 */
final class NoFailingExecutionFuzzingTest {
    ...
    private static void generateAndMask(
            JsonMaskingConfig jsonMaskingConfig,
            AtomicReference<String> lastExecutedJson,
            AtomicInteger randomTestsExecuted) {
    ...
    private static Stream<Arguments> failureFuzzingConfigurations() {
        ...
        return Stream.of(
                ...
                Arguments.of(JsonMaskingConfig.builder()
                        .maskKeys(targetKeys)
